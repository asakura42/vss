...
fetch_and_decode() {
	...
	if [[ "$type" == "b64" ]]; then
		curl -m "$CURL_TIMEOUT" -Ls "$url" | base64 -d | tee >(wc -l) | sed 's/$/\n/' >> "$output_file"
	else
		curl -m "$CURL_TIMEOUT" -Ls "$url" | tee >(wc -l) | sed 's/$/\n/' >> "$output_file"
	fi
}
...
if [[ -n "$json" ]]; then
	if geoiplookup  $(echo "$json" | jq -r '.add') | awk -F ',' '{print $1}' | awk -F ' ' '{print $NF}' | grep -Pv "$countries" 1>&2 ; then
...
if echo "$line" | grep -oP '(?<=ss:\/\/)[^@]+' | awk 'length > 80' | base64 -d 2>/dev/null | grep -q "2022-blake3\|ietf-poly1305" && 
    echo "$line" | grep -q ":443#" && 
    geoiplookup  $(echo "$line" | awk -F'@' '{print $2}' | awk -F':' '{print $1}') | awk -F ',' '{print $1}' | awk -F ' ' '{print $NF}' | grep -Pv "$countries" 1>&2 ; then
